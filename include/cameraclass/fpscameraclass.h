...
public:
    ...
    // returns the view matrix calculated using Euler Angles and the CreateViewMatrix function
    glm::mat4 GetViewMatrix()
    {
        return CreateViewMatrix(Position, Position + Front, Up);
    }

    glm::mat4 CreateViewMatrix(glm::vec3 position, glm::vec3 target, glm::vec3 up)
    {
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == RIGHT)
            Position += Right * velocity;
        // stop camera position from moving up or down the y axis
        Position.y = 0.0f;
    }
